...
type BrandManagerProps = {
  ...
  onUpdate?: (id: number, name: string, color: string, pkg: string, contactInfo: string, socialAccounts?: SocialMediaAccount[]) => void
}

export function BrandManager({ brands, loading, onCreate, onDelete, onUpdate }: BrandManagerProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingBrandId != null && typeof onUpdate === 'function') {
      onUpdate(editingBrandId, name.trim(), color, pkg.trim(), contactInfo.trim(), socialAccounts.length > 0 ? socialAccounts : undefined)
    } else if (editingBrandId != null) {
      // fallback to create if update not provided
  ...
  const startEdit = (brand: Brand) => {
    ...
    // brand may store package under 'package'
    // @ts-ignore
    setPkg((brand as any).package || '')
    setContactInfo(brand.contactInfo || '')
    setSocialAccounts(brand.socialAccounts ? [...brand.socialAccounts] : [])
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {loading ? 'Guardando...' : (editingBrandId ? 'Actualizar Marca' : 'Agregar Marca')}
            </button>
            {editingBrandId && (
              <button
                type="button"
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-blue-600 dark:text-blue-400 hover:text-blue-500 transition-colors p-1"
                    title="Editar marca"
                    aria-label="Editar marca"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M3 21v-3a2 2 0 0 1 .586-1.414L17.293 3.879a2 2 0 0 1 2.828 0l.707.707a2 2 0 0 1 0 2.828L6.121 21.414A2 2 0 0 1 4.707 21H3z"></path>
